...
/**
 * Shared page body for the apartments 楼盘 grid view.
 * Mounted at `/apartments` (main app) and `/rednote-factory/apartments` (RF).
 *
 * All internal navigation routes use the supplied basePath so the user
 * stays inside whichever shell they came from.
 */
export async function BuildingsGridPage({
  searchParams,
  basePath,
}: {
  searchParams: SP;
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col gap-3 px-3 py-3 md:gap-4 md:py-4 lg:p-8">
      ...
      <div className="sticky top-0 z-20 flex flex-wrap items-center gap-2 py-2">
        ...
        <div className="ml-auto">
          <ViewToggle
            current="buildings"
            searchParamsString={sp2qs(searchParams)}
            basePath={basePath}
          />
        </div>
      </div>
