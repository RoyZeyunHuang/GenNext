...
/**
 * Shared page body for the multi-building compare view.
 * Mounted at `/apartments/compare` and the RF equivalent.
 */
  ...
  if (buildings.length < 2) {
    return (
      <div className="mx-auto max-w-2xl p-6">
        <Link href={prefix} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:underline">
      ...
      <header className="flex flex-wrap items-baseline justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <CompareCopyButton
            ...
            schoolShort={school}
          />
          <Link href={prefix} className="rounded border px-3 py-1.5 text-xs hover:bg-accent">
            ← 返回
          </Link>
        </div>
      </header>
                ...
                <th key={b.id} className="border-l px-3 py-3 text-left">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0">
                      <Link href={`${prefix}/buildings/${b.building_slug ?? b.id}`}
