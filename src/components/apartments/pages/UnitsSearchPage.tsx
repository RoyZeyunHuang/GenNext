...
/**
 * Shared page body for the 房源 (unit search) view.
 * Mounted at `/apartments/units` (main) and `/rednote-factory/apartments/units` (RF).
 */
export async function UnitsSearchPage({
  searchParams,
  basePath,
}: {
  searchParams: SP;
  ...
  const moveIn =
    typeof searchParams.move_in === "string" ? searchParams.move_in : null;
  ...
  if (sort === "eff_rent_asc") {
    ...
    units = units.slice().sort((a, b) => {
      const ea =
        effectiveRent(a.price_monthly, a.months_free, a.lease_term_months) ??
        a.price_monthly ??
        Infinity;
      const eb =
        effectiveRent(b.price_monthly, b.months_free, b.lease_term_months) ??
        b.price_monthly ??
        Infinity;
      return ea - eb;
    });
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col gap-3 px-3 py-3 md:gap-4 md:py-4 lg:p-8">
      ...
      <div className="sticky top-0 z-20 flex flex-wrap items-center gap-2 py-2">
        ...
        <div className="ml-auto">
          <ViewToggle
            current="units"
            searchParamsString={qs.toString()}
            basePath={basePath}
          />
        </div>
      </div>
      ...
      {/* RESULTS */}
      <UnitTable units={(units ?? []) as never[]} basePath={basePath} />
    </div>
  );
}
